...
control = "/album";  ///나중에 board.js에서 가지고 있는 fuction과 연결  board/album으로 감 
...
function writeArticle() {
	if($("#subject").val() == "") {
		...
		document.writeForm.action = root + control + "/write.playdata"; //
		document.writeForm.submit(); //action
	}
}
</script>
...
<table border="0" cellpadding="0" cellspacing="0" width="630">
	<tr>
		...
		<td width="100%" style="padding-left: 4px"><b>글쓰기</b></td> <!--  버튼이 아니라 글자에 링크를 걸었음-->
	</tr>
	<tr>
...
<form id="writeForm" name="writeForm" method="post" action=""   도성줄펄 
	enctype="multipart/form-data" style="margin: 0px">
<div id="attach_file_hdn"></div>
<!-- album controller의 값이 여기에 각각 값이 대입된다 -->
<!--각각ㄱ 값이 대입되어 실행되는 코드를 작성하는 영역인데 인쿠르드 했기 때문에header_board.jsp으리 태그로전달 
모든 페이지에 include가 되어있음   /   또 준다이요"/write.playdata"; 일때 같이 넘어감  -->
	...
	<tr>
		...
		<a href="javascript:writeArticle();">
		 <img
			...
			border="0" name="register" value=""></a> <!--  버튼에 이미지 올리기   이미지가 없으면  alt="등록"가 표시됨-->  -->
			 <a href="javascript:history.back();">  <!--웹브라우니저의 아ㅠ으로 가라ㅓ 취소하면 전 페ㅣ지로   -->
			  <img
